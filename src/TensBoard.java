...
public class TensBoard extends Board
{
    ...
    {
		/*
		List<Integer> theCards = new ArrayList<>();
		for (int i = 0; i < cardIndexes().size(); i++)
		{
			theCards.add(i);
		}
		return containsJQK(theCards) || containsPairSum11(theCards);
		*/
        return containsQuartets(cardIndexes()) || containsPairSum10(cardIndexes());
    }
    ...
    private boolean containsPairSum10(List<Integer> selectedCards)
    {
        for (int i = 0; i < selectedCards.size() - 1; i++)
        {
            ...
            {
                if ((cardAt(selectedCards.get(i)).pointValue() + cardAt(selectedCards.get(k)).pointValue()) == 10)
                    return true;
            }
        }
        return false;
    ...
    private boolean containsQuartets(List<Integer> selectedCards)
    {
        ...
        for (int i = 0; i < selectedCards.size(); i++)
        {
            ...
            if (cardAt(selectedCards.get(i)).pointValue() == 12)
                queen++;
            if (cardAt(selectedCards.get(i)).pointValue() == 13)
                king++;
        }
        return tens == 4 || jack == 4 || queen == 4 || king == 4;
    }
}
